#!/usr/bin/env python3
"""
Word Shuffle Experiment on result.traj

This script:
1. Loads results.json with result.traj as reasoning
2. Shuffles reasoning by words
3. Generates new answers with shuffled reasoning
...
def prepare_shuffle_task(
    item: Dict,
    model_type: str,
    seed_base: int = 42
) -> Task:
    """
    Prepare a Task for shuffled reasoning generation
    ...
    # Shuffle reasoning
    shuffled_reasoning = shuffle_reasoning_words(
        original_reasoning,
        seed=seed_base + index
    )

    # Build prompt with shuffled reasoning
    prompt = build_gpt_oss_prompt_with_reasoning(question, shuffled_reasoning)
...
def run_experiment(
    ...
    # Prepare tasks
    print("Preparing shuffle tasks...")
    tasks = []
    for item in data:
        task = prepare_shuffle_task(item, model_type)
        tasks.append(task)

    # Phase 1: Generate answers with shuffled reasoning
    results = []
    print(f"\n=== Phase 1: Generating answers with shuffled reasoning ({len(tasks)} tasks) ===")
    ...
    print("\n" + "="*60)
    print("EXPERIMENT RESULTS - Word Shuffled Reasoning")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(
        description='Word Shuffle Experiment on result.traj'
    )
    parser.add_argument(
        '--results_path',
        type=str,
    ...
    parser.add_argument(
        '--output_path',
        type=str,
        default='data/baseline/shuffle_word_experiment.json',
